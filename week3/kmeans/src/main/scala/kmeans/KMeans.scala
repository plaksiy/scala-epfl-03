...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    (points, means) match {
      case (Nil, Nil) => GenMap()
      case (Nil, ms) => (for (m <- means) yield (m, GenSeq())).toMap
      case (ps, ms) => ps.groupBy(findClosest(_, ms))
    }
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(oldM => findAverage(oldM, classified(oldM)))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall{ case (o: Point, n: Point) => o.squareDistance(n) <= eta}
  }
