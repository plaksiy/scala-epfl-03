...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val _boundaries = new Boundaries()
    _boundaries.minX = Math.min(boundaries.minX, body.x)
    _boundaries.maxX = Math.max(boundaries.maxX, body.x)
    _boundaries.minY = Math.min(boundaries.minY, body.y)
    _boundaries.maxY = Math.max(boundaries.maxY, body.y)
    _boundaries
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val _boundaries = new Boundaries()
    _boundaries.minX = Math.min(a.minX, b.minX)
    _boundaries.maxX = Math.max(a.maxX, b.maxX)
    _boundaries.minY = Math.min(a.minY, b.minY)
    _boundaries.maxY = Math.max(a.maxY, b.maxY)
    _boundaries
  }
